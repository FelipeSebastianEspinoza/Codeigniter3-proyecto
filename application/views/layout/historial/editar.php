 				...
 				<div class="card-body">
 					<div>
 						...
 						<div class="form-group row" id="titulo">
 							...
 							<input type="hidden" id="inputIdTipo" name="tipo" class="form-control form-control-user" value="<?php echo $obj->tipo ?>">
 						...
 						<div class="form-group row" id="descripcion">
 							...
 							<div class="col-sm-6">
 								...
 								<?php if ($obj->estado != 'Terminado') {
									?>
 									<input type="radio" id="Terminado" name="estado" value="Terminado" checked>
 					...
 					<div class="col-sm-12 mb-6 mb-sm-0">
 						<table class=".table-sm table-striped">
 							...
 							<tbody>
 								<?php foreach ($historial_anexos as $arch) { ?>
 									<tr>
 										<td>
 										...
 										<td>
 											<a class="btn btn-danger btn-circle" data-toggle="modal" data-target="#deleteReporteModal" style="cursor: pointer;width:30px; height:30px;margin-right: 15px; margin-left: 5px;" onclick="javascript:document.getElementById('delete_reportearchivo').value=<?php echo $arch->id_anexos ?>">
 					...
 					<form id="delete_formreportearchivo">
 						<input type="hidden" id="delete_reportearchivo" name="id_anexos" value="">
 	...
 	<script>
 		$(document).ready(function() {
 			$('#uploadfile_form').on('submit', function(e) {
 				e.preventDefault();
 				$.ajax({
 					url: "<?php echo site_url() . '/historial/uploadfilehistorial' ?>",
 					method: "POST",
 					...
 					success: function(data) {
 						...
 						var $idH = document.getElementById("inputIdTipo").value;
 						if ($idH == 'seremi') {
 							window.location.href = '<?php echo site_url('historial/successhistorialmodificarseremi/') ?>';
 						} else {
 							window.location.href = '<?php echo site_url('historial/successhistorialmodificarmutual/') ?>';
 						}
 					},
 					statusCode: {
 						400: function(xhr) {
 	...
 	<script>
 		$(document).ready(function() {
 			$('#delete_formreportearchivo').on('submit', function(e) {
 				...
 				$.ajax({
 					type: 'POST',
 					url: "<?php echo site_url() . '/historial/eliminarArchivo' ?>",
 					data: $(this).serialize(),
 					success: function(data) {
 					 
						var $idH = document.getElementById("inputIdTipo").value;
 						if ($idH == 'seremi') {
 							window.location.href = '<?php echo site_url('historial/successdeletearchivoseremi/') ?>';
 						} else {
 							window.location.href = '<?php echo site_url('historial/successdeletearchivomutual/') ?>';
 						}
 					},
 				});
 			});
 	...
 	<script>
 		$(document).ready(function() {
 			$('#upload_form').on('submit', function(e) {
 				...
 				$.ajax({
 					type: 'POST',
 					url: "<?php echo site_url() . '/historial/modificarhistorialajax_upload' ?>",
 					data: $(this).serialize(),
 					success: function(data) {
