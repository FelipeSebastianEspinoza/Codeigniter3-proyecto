...
<div class="scalize w-100">
    ...
    <?php
    foreach ($redhumeda as $red) {
    ?>
        <?php if ($red->estado == 'Pendiente') {   ?>
            <div>
                <!--   <a  href="<?php echo site_url('redhumeda/editar/' . $red->id_redhumeda) ?>">  -->
                <div class='item-point' data-top='<?php echo $red->posx ?>' data-left='<?php echo $red->posy ?>' style=' background: #e70000; border: solid 3px #bf000a;' onmouseout='imagenNormal()' onmouseover="javascript:document.getElementById('myImage').src= '<?php echo base_url() . 'assets/upload/' .  $red->imagen ?>' ,document.getElementById('fotoTitulo').innerHTML = '<?php echo $red->nombre ?>'  ">
                </div>
                <!--    </a>  -->
            </div>
        <?php  } else {  ?>
